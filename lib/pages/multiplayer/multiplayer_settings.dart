...
import 'dart:math'; // Import this for generating unique IDs
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  _generateGameId() {
    // Generate a unique game ID (simple random string example)
    _gameId = List.generate(8, (_) => Random().nextInt(10).toString()).join();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Multiplayer Settings', style: TextStyle(color: Colors.white)),
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                await Clipboard.setData(ClipboardData(text: _gameId));
              },
              child: const Text('Copy Game ID', style: TextStyle(color: Colors.white)), // Text color
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.grey[850], // Button background color
                minimumSize: const Size(350, 36), // Button width and height
              ),
